...
namespace TMDT_Project.Models
{
    public sealed class PaypalClient
    {
        ...
        public async Task<CreateOrderResponse> CreateOrder(string value, string currency, string reference, List<GameVM> ListItems)
        {
            ...
            foreach (var element in ListItems)
            {
                var item = new item
                {
                    ...
                    unit_amount = new UnitAmount { currency_code=currency, value = element.CurrPrice.ToString()},
                    //price = element.CurrPrice.ToString(),
                    quantity = "1",
                    sku = "SKU02",
                };
                cartItems.Add(item);
